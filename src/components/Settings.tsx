...
export default function Settings() {
  ...
  const [isInitialLoading, setIsInitialLoading] = useState(false); // Start as false to prevent hydration mismatch
  ...
  const [mounted, setMounted] = useState(false); // Add mounted state to track client-side hydration
  ...
  useEffect(() => {
    setMounted(true);
    setIsInitialLoading(true); // Set loading state after component mounts
  }, []);

  // Auto-refresh data when component mounts or when returning from OAuth
  useEffect(() => {
    if (!mounted) return; // Only run after component is mounted on client
  ...
  const handleGoogleSignIn = () => {
    // Store the current referrer or default to chat page
    const referrer = document.referrer || '/chat';
    sessionStorage.setItem('oauth_return_url', referrer);
  ...
  const handleBack = () => {
    // Check if we have a stored return URL from OAuth flow
    const returnUrl = sessionStorage.getItem('settings_return_url');
    if (returnUrl) {
  ...
  // Don't render main content until mounted to prevent hydration mismatch
  if (!mounted) {
    return (
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-medium text-gray-900 mb-6">Google Account</h2>
